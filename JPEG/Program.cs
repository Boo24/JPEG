...
//				var fileName = "Big_Black_River_Railroad_Bridge.bmp";
	    ...
	    private static CompressedImage Compress(CbCrImage cbCrImage, int quality = 50)
	    {
	        ...
	        var channels = new List<double[,]> { cbCrImage.y, cbCrImage.Cb, cbCrImage.Cr };
	        ...
	        return new CompressedImage { Quality = quality, CompressedBytes = compressedBytes, BitsCount = bitsCount, DecodeTable = decodeTable, Height = cbCrImage.Height, Width = cbCrImage.Width };
	    }
	    ...
	    private static double[,] GetSubMatrix(double[,] matrix, int yOffset, int yLength, int xOffset, int xLength, Func<double, double> f)
	    {
	        var result = new double[yLength, xLength];
	        for (var j = 0; j < yLength; j++)
	        for (var i = 0; i < xLength; i++)
	            result[j, i] = f(matrix[yOffset + j, xOffset + i]);
	        return result;
	    }


        private static List<byte> ZigZagScan(byte[,] channelFreqs)
		{
